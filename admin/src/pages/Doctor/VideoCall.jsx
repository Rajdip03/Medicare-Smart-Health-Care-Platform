...
const VideoCall = () => {
  ...
  const preparePeerConnection = useCallback((socket) => {
    ...
    pc.ontrack = (event) => {
      console.debug('pc.ontrack event (doctor):', event)
      let stream = null
      ...
      if (stream) {
        ...
        console.debug('Doctor remote stream tracks:', tracks.map(t => ({ kind: t.kind, id: t.id })))
        setRemoteTracksCount(tracks.length)
  ...
  const initCall = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(
        `${backendUrl}/api/doctor/video-room/${meetingRoomId}`,
      ...
      console.debug('Doctor local stream tracks:', localStream.getTracks().map(t => ({ kind: t.kind, id: t.id })))
      ...
      socket.on('connect', () => {
        socket.emit('join-room', { roomId: meetingRoomId, userId: data.appointment.docId, role: 'doctor' })
      })
      ...
      socket.on('user-disconnected', () => {
        toast.info('Patient disconnected')
      })
    } catch (error) {
      console.error(error)
  ...
  }, [backendUrl, dToken, meetingRoomId, navigate, preparePeerConnection])
      ...
      <div className='mx-auto max-w-6xl rounded-3xl bg-white shadow-xl p-6'>
        <div className='flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
          <div>
            ...
            <h1 className='text-2xl font-bold text-slate-900'>Doctor Video Consultation</h1>
            <p className='text-sm text-slate-600'>Patient: {appointment.userData?.name || 'Patient'}</p>
            <p className='text-sm text-slate-600'>Doctor: {appointment.docData?.name || 'You'}</p>
          </div>
        ...
        <div className='mt-8 grid gap-4 md:grid-cols-2'>
          ...
          <div className='rounded-2xl bg-slate-900 p-4'>
            <p className='text-sm text-slate-300 mb-2'>Patient</p>
        ...
        <div className='mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4'>
          ...
          <p className='text-sm text-slate-600 mt-2'>Remote tracks: <span className='font-semibold'>{remoteTracksCount}</span></p>
          <p className='text-sm text-slate-600 mt-2'>Use the controls to mute audio, disable camera, or end the consultation.</p>
        </div>
      </div>
    </div>
  )
